...
def decorate(
    prompt_fn, _func=None, *, log: Optional[FunctionLogHandler]=None, llm: Optional[LLM]=None, error: Optional[ErrorHandler]=None, 
):
    def decorator(
        func: Fn,
...
def prompt(
    _func=None, *, log: Optional[FunctionLogHandler]=None, llm: Optional[LLM]=None, error: Optional[ErrorHandler]=None, 
    ...
    async def prompt_fn(wrapper_fn, decorated_fn: Fn, original_fn: Fn, *args, **kwargs) -> Awaitable[Any]:
        llm = get_llm(wrapper_fn)
        diagraph_log: Optional[FunctionLogHandler] = getattr(wrapper_fn, "__diagraph_log__", None)
        diagraph_error: Optional[ErrorHandler] = getattr(wrapper_fn, "__diagraph_error__", None)
        ...
        try:
            ...
            if error:
                ...
                raise e
    return decorate(prompt_fn, _func, log=log, llm=llm, error=error)
